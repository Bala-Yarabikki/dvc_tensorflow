...
def copy_file(source_download_dir, local_data_dir):
    ...
    N = len(list_of_files)
    for file in tqdm(list_of_files,N,desc=f"copying file from {source_download_dir} to {local_data_dir}",colour="green"):
        src_file = os.path.join(source_download_dir, file)
