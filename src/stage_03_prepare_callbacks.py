...
from src.utils.callbacks import create_and_save_tensorboard_callback, create_and_save_checkpoint
import argparse
import os
import logging
...
def prepare_callbacks(config_path,params_path):
    ...
    create_and_save_checkpoint(callback_dir,checkpoint_dir)
...
if __name__ == '__main__':
    ...
    try:
        logging.info(">>>>>stage 03 starting")
        prepare_callbacks(config_path=parsed_args.config, params_path=parsed_args.params)
        logging.info("stage 03 completed! callbacks are prepared and saved as binary >>>>>>")
